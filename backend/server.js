...
async function handleRegister(req, res) {
  ...
  if (username.length < 3 || password.length < 6) {
    send(res, 400, { error: "Username minimo 3 caratteri, password minimo 6." });
    return;
  }

  const data = readUsers();
  if (findUser(data, username)) {
    send(res, 409, { error: "Username gia registrato." });
    return;
  }
...
async function handleLogin(req, res) {
  ...
  if (!user || !verifyPassword(password, user)) {
    send(res, 401, { error: "Credenziali non valide." });
    return;
  }
...
function handleGetProfile(req, res) {
  ...
  if (!username) {
    send(res, 401, { error: "Token mancante o non valido." });
    return;
  }
  ...
  const user = findUser(data, username);
  if (!user) {
    send(res, 404, { error: "Utente non trovato." });
    return;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (req.method === "GET" && req.url === "/health") {
      ...
      send(res, 404, { error: "Endpoint non trovato." });
    }
  } catch (error) {
    send(res, 500, { error: error.message || "Errore server." });
  }
});
